...
OpenRelTable::OpenRelTable()
{
  ...
  for (int relId = RELCAT_RELID; relId <= ATTRCAT_RELID; relId++)
  {
    ...
    *(RelCacheTable::relCache[relId]) = relCacheEntry;                                  // copying constructed entry to cache
    ...
    memcpy(tableMetaInfo[relId].relName, relCacheEntry.relCatEntry.relName, ATTR_SIZE); // relation name stored
  }
  ...
  // LL for storing attrcache entries for the relcatentries
  auto rel_cat_listhead = createAttrCacheEntryList(RELCAT_NO_ATTRS);
  auto attr_cache_entry = rel_cat_listhead;

  // populate attrcache with entries corresponding to relcatattr
  for (int i = 0; i < RELCAT_NO_ATTRS; i++)
  {
    ...
    AttrCacheTable::recordToAttrCatEntry(attrCatRecord, &(attr_cache_entry->attrCatEntry));
    (attr_cache_entry->recId).block = ATTRCAT_BLOCK; // Assign block number to recId
    (attr_cache_entry->recId).slot = i;              // Assign slot number to recId

    attr_cache_entry = attr_cache_entry->next; // Move to the next entry in the list
  }
  AttrCacheTable::attrCache[RELCAT_RELID] = rel_cat_listhead; // Store the head of the list in the attribute cache
  ...
  attr_cache_entry = attrCatListHead;
...
OpenRelTable::~OpenRelTable()
{
  ...
  for (int i = 2; i < MAX_OPEN; ++i)
  {
    ...
    for (int i = 0; i < MAX_OPEN; i++)
    {
      free(RelCacheTable::relCache[i]);        // Free memory allocated for relcache
      clearList(AttrCacheTable::attrCache[i]); // Free memory for the LL in attrcache
      RelCacheTable::relCache[i] = nullptr;    // Reset relcache entry to nullptr
      AttrCacheTable::attrCache[i] = nullptr;  // Reset attrcache entry to nullptr
    }
  }
